...
object Event {
    ...
    DB.withConnection {
      implicit connection =>
        SQL("select * from " + EVENTS_TABLE_NAME + " e " +
          "left join " + Attendee.ATTENDEES_TABLE_NAME + " a on e." + ID + "=a." + Attendee.EVENT_ID + " " +
  ...
  def list(friendsList: List[String]): List[(Event, Option[Attendee])] = {
    val query =
      "select * from " + EVENTS_TABLE_NAME + " e, " + Attendee.ATTENDEES_TABLE_NAME + " a where e." + ID + " = a." + Attendee.EVENT_ID + " and (1 = 0 " +
  ...
  def populate(): List[Event] = {
    DB.withConnection {
      implicit connection =>
        SQL("select * from " + EVENTS_TABLE_NAME).as(detailDeserialise *)
    }
  }

  def populateWithAttendingStatus(userId: String): List[Event] = {
    ...
    val _parser = {
      get[Long](ID) ~
        ...
        get[String](SHORT_DESC) ~
        get[Double](RATING) ~
        get[Option[Long]](Attendee.ID) ~
        get[Option[String]](Attendee.USER_ID) ~
        get[Option[Long]](Attendee.EVENT_ID) map {
        case id ~ name ~ address ~
          latitude ~ longitude ~ desc ~ shortDesc ~ rating ~ attendingId ~ attendingUserId ~ attendingEventId => {
          Event(id, name, address,
            latitude, longitude,
            attendingId != null && attendingUserId.getOrElse(false).equals(userId),
            desc, shortDesc, rating)
        }
      }
    }
    ...
    DB.withConnection {
      implicit connection =>
        val query = "select * from " + EVENTS_TABLE_NAME + " e left join " +
          Attendee.ATTENDEES_TABLE_NAME + " a on " +
          " e." + ID + "=a." + Attendee.EVENT_ID
        val res = SQL(query
        )
        res.as(_parser *).groupBy(_.id).map(_._2.head).toList
    }
  }
}
